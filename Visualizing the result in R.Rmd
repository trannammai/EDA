---
title: "Rapport final du cours SQL"
author: "SALOMON HAZAN, CLARA LUCAS, TRAN NAM MAI"
date: "May 9, 2020"
output:
  html_notebook: 
    toc: true
    toc_float: true
---
L'objectif de ce rapport est de visualiser les données exportées à partir des requêtes SQL issues du projet transverse - Partie SQL. Ce rapport comprend 3 parties principales :
 
    I. Etude globale
    1. Répartition client
    2. Comportement du chiffre d'affaires par client en N-2 (2016) et N-1 (2017)

    II. Etude par magasin
    1.  Evolution en terme du nombre des clients actifs et chiffre d'affaires par magasin
    2.  Distance entre domicile client et magasin

    III. Etude par univers
    1.   Evolution du chiffre d'affaires par univers en N-2 (2016) et N-1 (2017)
    2.   Top 5 des familles les plus rentables par univers

Dans chaque partie, nous fournissons la requête SQL ainsi que les données exportées à partir de ces requêtes et les visualisations qui sont élaborées dans R.

Attention : N’exécuter ce Rmarkdow que partie par partie, car les parties contenant des requêtes SQL renverront des erreurs. Si on exécute une partie une seconde fois, il y aura une erreur (relancer l’importation des données au préalable)
...
```
> La proportion de churner est la plus importante (34,79%) suivie du nombre d'adhérents (22,53%), des nouveaux adhérents 2016 (15,45%), et des nouveaux adhérents 2017 (14,18%). La proportion de VIP n'est que de 13,6%.

## 2. Comportement du CA GLOBAL par client N-2 vs N-1
### Requête SQL
```{sql}
select *
from(
...
> Dans le premier graphique le comportement des clients en terme de CA  est difficile à visualiser du fait des valeurs extrêmes (outliers).
Le deuxième graphique permet de mieux mettre en évidence la médiane du CA qui passe de 199 euros à 246 euros (le CA au-dessous duquel se situe 50 % des achats clients). La moyenne des achats après exclusion des outliers passe de 265 euros à 316 euros.

## 3. Répartition par âge et par sexe
> Nous avons décidé de limiter la population à représenter afin d'avoir une meilleure distribution et une meilleure visibilité de la variable âge. Les clients étudiés étant des adhérents, nous avons exclu les moins de 18 ans et les plus de 100 ans.
